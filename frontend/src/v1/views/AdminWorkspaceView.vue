  ...
  <section class="dashboard-page">
    ...
    <template v-else-if="section === '知识图谱治理'">
      <div class="two-column admin-layout graph-admin-layout">
        <section class="panel"><div class="panel-head"><div><p class="eyebrow">图谱版本</p><h3>{{ selectedCourse?.courseName }}</h3></div></div><article v-for="version in versions" :key="version.id" class="admin-list-card" :class="{ selected: selectedVersion?.id === version.id }" @click="openVersion(version.id)"><div><strong>版本 {{ version.versionNo }}</strong><span>{{ version.description || '暂无版本说明' }}</span></div><div><span class="soft-badge">{{ statusText(version.status) }}</span><small v-if="version.active">当前在线</small></div></article><form class="inline-create" @submit.prevent="createVersion"><input v-model="newVersionDescription" placeholder="新版本说明"><button class="primary-button small">从当前图创建草稿</button></form></section>
